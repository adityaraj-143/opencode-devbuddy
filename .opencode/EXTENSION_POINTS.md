# Extension Points for Dev-Buddy Integration

## CRITICAL FILE — This identifies safe integration locations for Dev-Buddy.
...
events.subscribe(SessionEvent.*, (event) => {
  // Dev-Buddy: detect session start, load project memory
})
```
...
events.beforeCommit("dev-intel/session-start", (event) => {
  // Dev-Buddy: run before session event is committed to DB
})
```
...
| `SystemContext.builtins` | `packages/core/src/system-context/builtins.ts` | Add Dev-Buddy built-in sources here | **SAFE** |
...
events.subscribe(SessionEvent.Tool.Called, (event) => {
  DevBuddy.onToolStart(event.sessionID, event.callID, event.name)
})
events.subscribe(SessionEvent.Tool.Success, (event) => {
  DevBuddy.onToolFinish(event.sessionID, event.callID)
})
events.subscribe(SessionEvent.Tool.Failed, (event) => {
  DevBuddy.onToolError(event.sessionID, event.callID, event.error)
})
```

Or via `Tool.make()` wrapper:
```ts
const DevBuddyTool = Tool.make({
  ...originalTool,
  execute: (input, ctx) => {
    DevBuddy.notifyToolStart(originalTool.id, input)
    return originalTool.execute(input, ctx).pipe(
      Effect.tapBoth({
        onSuccess: (result) => DevBuddy.notifyToolSuccess(originalTool.id, result),
        onFailure: (error) => DevBuddy.notifyToolError(originalTool.id, error),
      })
    )
  }
})
...
### Where Dev-Buddy can persist information

| Point | File | Description | Safety |
|-------|------|-------------|--------|
| `EventV2.publish()` | `packages/core/src/event.ts` | Publish Dev-Buddy events into the event log | **SAFE** — Event sourcing is append-only |
...
| New Drizzle tables | `packages/core/src/**/*.sql.ts` | Create new tables for Dev-Buddy data (project memory, developer context, etc.) | **SAFE** — Follow existing pattern |
| `Config.Service` | `packages/core/src/config.ts` | Config storage for Dev-Buddy settings | **SAFE** |
...
export const DevBuddyMemoryTable = sqliteTable("dev_intel_memory", {
  id: text().primaryKey(),
...
### Where Dev-Buddy can expose UI elements
...
**Recommended: Create a Dev-Buddy sidebar plugin**

```ts
// Following patterns in feature-plugins/sidebar/context.tsx
class DevBuddySidebarPlugin implements BuiltinTuiPlugin {
  id = "dev-intel-memory-sidebar"
  setup(api: TuiPluginApi) {
    api.slots.register("sidebar_content", () => <DevBuddyMemoryPanel />)
  }
}
```
...
| Point | File | Description | Safety |
|-------|------|-------------|--------|
| Session context tab | `packages/app/src/components/session/session-context-tab.tsx` | Context breakdown (extend with Dev-Buddy info) | **MODERATE** — UI change in app |
...
| `EventV2.project()` | `packages/core/src/event.ts` | Register a projector for Dev-Buddy events | **SAFE** |
...
| Point | File | Description | Safety |
|-------|------|-------------|--------|
| `AppRuntime` layers | `packages/opencode/src/effect/app-runtime.ts` | Add Dev-Buddy service Layer | **SAFE** — Layer.mergeAll pattern |
| `Plugin.Service` | `packages/opencode/src/plugin/` | Plugin system hooks | **SAFE** |
| `Global.defaultLayer` | `packages/tui/src/app.tsx` | TUI layer where Dev-Buddy can be provided | **SAFE** |
...
| Dev-Buddy persistence | New Drizzle table in `packages/core/src/**/*.sql.ts` | SAFE | Medium |
