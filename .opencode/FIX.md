...
- **Not caused by our code**: The crash reproduced on the parent commit (`85e278b72` "sync release versions for v1.17.7") which has **zero** devintel changes.
...
- **Actually WAS our imports**: After rebasing onto the real `v1.17.7`, the crash persisted — ultimately traced to the two static `import` statements we added for devintel integration.

## Root Cause

### The Immediate Trigger

Two static import lines pulled `@opencode-ai/devintel` into the app's module graph at **load time** (before any code ran):
...
Layer.provideMerge(
  Layer.unwrap(
    Effect.promise(async () => {
      const { DevIntelLayer } = await import("@opencode-ai/devintel")
      return DevIntelLayer.layer
    }),
  ) as Layer.Layer<any, never, never>,
)
```

**Why it failed**: `ManagedRuntime.make(AppLayer)` eagerly builds all layers, including unwrapped ones. The `Effect.promise` was evaluated during layer construction at startup, which triggered the same dynamic `import()`, loading devintel and its `@opentui` dependencies at startup time anyway.

### 2. Dynamic import in `createBuiltinPlugins`

Attempting `await import("@opencode-ai/devintel/tui")` inside the `createBuiltinPlugins()` function also failed because the function was called early enough in the TUI startup that the crash still occurred.

## The Working Fix

### Strategy: Defer the import to plugin activation time

Instead of loading devintel at module load time or even at layer construction time, defer it to when the TUI plugin is **actually activated** — when the user navigates to the devintel sidebar panel.
...
Removed the devintel layer entirely from `AppLayer`. Since the devintel services (`DevIntelDb`, `DevIntelProjects`) are only needed when the TUI plugin runs, not at app startup, they can be created on-demand by the plugin when it initializes. This avoids the `Layer.unwrap` issue where `ManagedRuntime.make` eagerly evaluates all layers.
...
Module resolution in JavaScript is eager by default — static `import` declarations are evaluated when the importing module is first loaded. Dynamic `import()` expressions are evaluated when the code reaches them at runtime. By moving the devintel import from "module load time" to "plugin activation time", we avoided the bun 1.3.14 terminal initialization bug entirely.
...
- Dev-Intel TUI plugin is registered in the builtins list
- The plugin is only loaded when the user navigates to the devintel sidebar panel
- All 23 turbo typecheck tasks pass
- 4 devintel smoke tests pass with in-memory SQLite
