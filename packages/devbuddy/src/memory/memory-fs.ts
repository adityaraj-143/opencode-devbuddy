...
export const DEVBUDDY_DIR = ".devbuddy"
...
function devBuddyPath(worktreePath: string): string {
  return join(worktreePath, DEVBUDDY_DIR)
}

function filePath(worktreePath: string, name: string): string {
  return join(devBuddyPath(worktreePath), name)
}

export async function ensureDir(worktreePath: string): Promise<void> {
  await mkdir(devBuddyPath(worktreePath), { recursive: true })
}
