export * as DevBuddyProjectMemory from "./project-memory"
...
export class Service extends Context.Service<Service, Interface>()("@opencode/v2/devbuddy/ProjectMemory") {}
...
const _initialize = Effect.fn("DevBuddyProjectMemory.initialize")(function* (projectId: string) {
...
const _getProjectMemory = Effect.fn("DevBuddyProjectMemory.getProjectMemory")(function* (projectId: string) {
  const path = yield* worktreePath(projectId)
...
const _updateMetadata = Effect.fn("DevBuddyProjectMemory.updateMetadata")(function* (
  projectId: string,
  input: { name?: string; description?: string; goals?: string[]; constraints?: string[] },
) {
  const path = yield* worktreePath(projectId)
...
const _addTask = Effect.fn("DevBuddyProjectMemory.addTask")(function* (
  projectId: string,
...
const _updateTask = Effect.fn("DevBuddyProjectMemory.updateTask")(function* (
  projectId: string,
  taskId: string,
  input: { title?: string; description?: string; status?: TaskStatus },
) {
...
const _addDecision = Effect.fn("DevBuddyProjectMemory.addDecision")(function* (
  projectId: string,
...
const _addNote = Effect.fn("DevBuddyProjectMemory.addNote")(function* (
  projectId: string,
  content: string,
) {
  const path = yield* worktreePath(projectId)
